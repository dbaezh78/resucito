...
const getInfoLiturgicaActual = () => {
    ...
    for (const tiempo of tiemposConSemanas) {
        let semanasDelTiempo;
        
        if (tiempo === 'Ordinario') {
            semanasDelTiempo = ordinaryTimeWeeks[cicloActual];
        } else {
            semanasDelTiempo = ordinaryTimeWeeks[tiempo];
        }

        if (semanasDelTiempo) {
            for (const semana of semanasDelTiempo) {
                ...
                // La corrección clave: la semana se identifica si la fecha está entre el inicio y el fin (ambos inclusive)
                if (todayUTC >= inicioUTC && todayUTC <= finUTC) {
                    infoActual = {
                        tiempo: (tiempo === 'Ordinario' ? 'Tiempo Ordinario' : tiempo),
                        codigo: (tiempo === 'Ordinario' ? 'to' : tiempo.toLowerCase())
                    };
