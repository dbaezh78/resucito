...
// 4. COMPLETAR DATOS: El "Escudo" inteligente
async function completarDatosLentamente(cantos) {
    const user = auth.currentUser;
    if (!user) return;
    
    // Leemos si la sincronización está activa (por defecto sí)
    const syncActiva = document.getElementById('syncToggle').checked;

    for (const canto of cantos) {
        // Primero: Intentamos leer siempre del teléfono (Cero costo)
        const localData = localStorage.getItem(`data-${canto.id}`);
        
        if (localData) {
            // Si ya lo tenemos, lo pintamos de inmediato
            inyectarDatosEnTabla(canto.id, JSON.parse(localData), true);
        } 
        
        // Segundo: Solo si NO lo tenemos local O si la sincronización está activa, vamos a Firebase
        if (!localData || syncActiva) {
            // Si ya lo tenemos local pero la sincronización está activa, 
            // solo pedimos a Firebase de vez en cuando (puedes ajustar el tiempo)
