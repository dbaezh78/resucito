...
function actualizarInterfazSeleccion() {
    // 1. Actualizar el número del contador inferior
    const contador = document.getElementById('contador-seleccion');
    if (contador) contador.innerText = listaOrdenada.length;

    // 2. Renderizar las burbujas (tags) en la cola de selección
    ...
    if (cola) {
        ...
        listaOrdenada.forEach((id, i) => {
            ...
            if (canto) {
                ...
                // Si haces clic en la burbuja, también se deselecciona
                tag.onclick = (e) => {
                    e.stopPropagation();
                    window.toggleCanto(id);
                };
    ...
    // 3. SINCRONIZAR LOS INTERRUPTORES (SWITCHES) VISUALES
    // Esto es lo que hace que el check se mueva a la posición de "encendido"
    const inputs = document.querySelectorAll('.item-canto input[type="checkbox"]');
    inputs.forEach(input => {
        const idInput = input.getAttribute('data-id');
        // El input se marca como 'checked' solo si su ID está en nuestra lista ordenada
