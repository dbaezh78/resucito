...
const getInfoLiturgicaActual = () => {
        ...
        for (const tiempo of tiempos) {
            if (tiempo.tipo === 'semanas') {
                ...
                if (today >= inicio && today <= fin) {
                    ...
                    if (tiempo.codigo === 'to') {
                        // Es Tiempo Ordinario - buscar en ordinaryTimeWeeks
 ...
 const displaySemanas = (tiempo) => {
    ...
    if (tiempo.tipo === 'semanas') {
        ...
        if (tiempo.codigo === 'to') {
            // Tiempo Ordinario - usar ordinaryTimeWeeks
            semanas = ordinaryTimeWeeks[selectedCiclo] || [];
        } else {
            // Para otros tiempos (Adviento, Cuaresma, Pascua, Navidad)
            // Buscar en ordinaryTimeWeeks usando el nombre del tiempo
            const tiempoKey = tiempo.tiempo;
            semanas = ordinaryTimeWeeks[tiempoKey] || [];
            
            // Si no se encuentran, generar semanas básicas
            if (semanas.length === 0) {
        ...
        semanas.forEach(semana => {
            ...
            semanaBtn.addEventListener('click', () => {
                // Navegar inmediatamente al hacer clic en la semana
        ...
        // Lógica para las fiestas (esta parte ya funciona correctamente)
