...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        onSnapshot(q, (snapshot) => {
            // Evitamos parpadeos si viene de la caché de Firebase
            ...
            // Si no estamos buscando nada, actualizamos la UI
            const buscadorListas = document.getElementById('inputBuscadorListas');
    ...
    if (listas.length === 0) {
        contenedor.innerHTML = `
            <div class="status-msg-vacia">
                ...
                <a href="javascript:void(0)" onclick="window.irANuevaLista()" class="link-crear-lista">
                    ¿Deseas crearla?
                </a>
            </div>
        `;
...
window.guardarListaFirebase = async () => {
    ...
    // Mitigamos el error de "no aparece": limpiamos buscador y renderizamos al instante
    const buscadorListas = document.getElementById('inputBuscadorListas');
    if (buscadorListas) buscadorListas.value = '';
    renderizarListasUI(listasLocalesCache);
...
window.compartirListaLink = (idLista) => {
    ...
    try {
        ...
        navigator.clipboard.writeText(urlCompartir).then(() => alert("¡Enlace copiado, puedes compartirlo!"));
...
window.importarLista = (event) => {
    ...
    reader.onload = async (e) => {
        try {
            ...
            l.id = "imp-" + Date.now();
            l.nombre = "📂 " + l.nombre;
            listasLocalesCache.unshift(l);
            localStorage.setItem('cache_listas_personalizadas', JSON.stringify(listasLocalesCache));
            
            // Limpiamos buscador para que la lista importada aparezca arriba
            if (document.getElementById('inputBuscadorListas')) document.getElementById('inputBuscadorListas').value = '';
            renderizarListasUI(listasLocalesCache);
...
window.toggleDetalleLista = (idLista) => {
    ...
    if (estaCerrado) {
        ...
        // Buscamos en el snapshot o en el objeto simulado offline
        const docSnap = (snapshotActual.docs || snapshotActual).find(d => d.id === idLista);
...
(async () => {
    ...
    if (share) {
        ...
        } catch (e) { console.error("Error en link"); }
    }
})();