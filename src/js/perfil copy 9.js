...
// 5: INYECTAR DATOS: Adaptada para la estructura unificada dbdata
window.inyectarDatosEnTabla = function(cantoId, data, esLocal = false) {
    ...
    // --- 4. ACTUALIZAR FECHA DE USO ---
    // IMPORTANTE: En dbdata el campo se llama 'fecha', no 'valor'
    const fechaOrigen = data.fecha || data.valor; 
    if (elUso && fechaOrigen) {
        // Manejamos si viene como Timestamp de Firebase o Date
...
window.sincronizarTodoARam = async function() { 
    ...
    try {
        ...
        // 1. Petición masiva a la NUEVA colección dbdata
        ...
        querySnapshot.forEach((docSnap) => {
            ...
            // EL NUEVO DETECTIVE: Estructura plana dbdata {fecha, acorde, cejilla}
            const d = rawData.fecha || rawData.valor; // Compatibilidad temporal
            let fechaFinal = null;
            if (d) {
                fechaFinal = d.toDate ? d.toDate() : new Date(d);
            }

            // Normalizamos para que la RAM y la Tabla entiendan los datos
            const datosNormalizados = {
                valor: (fechaFinal && !isNaN(fechaFinal.getTime())) ? fechaFinal : null,
                fecha: (fechaFinal && !isNaN(fechaFinal.getTime())) ? fechaFinal : null,
                acorde: rawData.acorde || "0",
                cejilla: rawData.cejilla || "0"
            };

            // Guardar en RAM y LocalStorage (El Escudo)
            ...
            if (texto) texto.innerText = `Sincronizando dbdata: ${procesados} de ${total} cantos...`;
        });
