document.addEventListener('DOMContentLoaded', () => {
    ...
    const mesesContainer = document.getElementById('meses-container'); // Nuevo contenedor para los meses

    let selectedCiclo = 'C'; 
    let selectedTiempo = null;
    let selectedItem = null; // Unificado para la selección de semana o fiesta
    ...
    const displayMeses = () => {
        semanasContainer.innerHTML = '';
        const fiestasPorMes = selectedTiempo.fiestas;
        const mesesDiv = document.createElement('div');
        mesesDiv.classList.add('semana-botones'); // Reutilizamos la clase para el estilo
        semanasContainer.appendChild(mesesDiv);

        Object.keys(fiestasPorMes).forEach(mes => {
            const btn = document.createElement('button');
            btn.classList.add('semana-btn');
            btn.textContent = mes.charAt(0).toUpperCase() + mes.slice(1);
            btn.dataset.mes = mes;
            btn.addEventListener('click', () => {
                displayFiestasDeMes(mes);
            });
            mesesDiv.appendChild(btn);
        });
    };
    
    const displayFiestasDeMes = (mes) => {
        semanasContainer.innerHTML = '';
        const fiestasDelMes = selectedTiempo.fiestas[mes];
        const fiestasDiv = document.createElement('div');
        fiestasDiv.classList.add('semana-botones');
        semanasContainer.appendChild(fiestasDiv);

        // Botón para volver
        const backBtn = document.createElement('button');
        backBtn.textContent = '← Volver a Meses';
        backBtn.classList.add('back-btn');
        backBtn.addEventListener('click', displayMeses);
        semanasContainer.prepend(backBtn);

        if (fiestasDelMes && fiestasDelMes.length > 0) {
            ...
            fiestasDiv.innerHTML = '<p>No hay fiestas definidas para este mes.</p>';
        }
    };
    ...
    const displaySemanas = (tiempo) => {
        ...
        for (let i = 1; i <= tiempo.semanas; i++) {
            ...
            btn.addEventListener('click', () => {
                document.querySelectorAll('.semana-btn').forEach(b => b.classList.remove('selected'));
                btn.classList.add('selected');
                selectedItem = { numero: i, tipo: 'semana' };
            });
    ...
    goButton.addEventListener('click', () => {
        ...
        if (selectedTiempo.tipo === 'fiestas') {
            if (!selectedItem) {
                alert('La paz de Cristo Hermano,\nPor favor, selecciona una fiesta.');
                return;
            }
            if (selectedItem.url) {
                url += selectedItem.url;
            } else {
                alert('URL de la fiesta no definida.');
                return;
            }
        } else {
            if (!selectedItem) {
            ...
            url += `ai${selectedTiempo.codigo}s${selectedItem.numero}${selectedCiclo.toLowerCase()}`;
        }
