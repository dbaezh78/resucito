document.addEventListener('DOMContentLoaded', () => {
        ...
        if (tiempo.tipo === 'semanas') {
            ...
            if (tiempo.fiestas && tiempo.fiestas.length > 0) {
                tiempo.fiestas.forEach(fiesta => {
                    ...
                    // Almacenamos la URL especial si existe
                    if (fiesta.url) {
                    ...
                    btn.addEventListener('click', () => {
                        document.querySelectorAll('.semana-btn').forEach(b => b.classList.remove('selected'));
                        btn.classList.add('selected');
                        // Almacenamos el objeto de la fiesta seleccionada
                        selectedSemana = fiesta; 
                    });
    ...
    goButton.addEventListener('click', () => {
        ...
        if (selectedTiempo.tipo === 'fiestas') {
            // Revisa si la fiesta seleccionada tiene una URL especial
            if (selectedSemana && selectedSemana.url) {
                url += selectedSemana.url;
            } else {
                // Si no, usa el formato genérico
