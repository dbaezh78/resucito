...
// Variable para controlar el momento activo
let momentoActivo = 'Libre'; // 'Entrada', 'Paz', 'Comunión', 'Final', 'Libre'
...
window.actualizarInterfazSeleccion = () => {
    // 1. Actualizamos el contador total
    const contador = document.getElementById('contador-seleccion');
    if (contador) contador.innerText = listaOrdenada.length;

    // 2. Renderizamos la cola
    const cola = document.getElementById('cola-seleccion');
    if (cola) {
        cola.innerHTML = '';
        listaOrdenada.forEach((item) => {
            // Buscamos el canto
            const canto = todosLosCantos.find(c => String(c.id) === String(item.id));
            
            if (canto) {
                const tag = document.createElement('div');
                tag.className = 'canto-tag';
                
                // IMPORTANTE: item.etiqueta ya es el string (E, P, C, F o numero)
                tag.innerHTML = `<strong>${item.etiqueta || ''}</strong> ${canto.title}`;
                
                tag.onclick = (e) => { e.stopPropagation(); window.toggleCanto(item.id); };
                cola.appendChild(tag);
            }
        });
    }

    // 3. Sincronización de checkboxes (mantenemos tu lógica con .some)
    document.querySelectorAll('.item-canto input[type="checkbox"]').forEach(input => {
        const idInput = input.getAttribute('data-id');
        input.checked = listaOrdenada.some(item => String(item.id) === String(idInput));
    });
};
...
window.guardarListaFirebase = async (btn) => {
    ...
    const nuevaLista = { 
        ...
        ids_cantos: listaOrdenada.map(item => ({ id: item.id, tag: item.etiqueta })),
...
window.importarLista = (event) => {
    ...
    reader.onload = async (e) => {
        try {
            ...
            // LIMPIEZA: Aseguramos que la lista se guarde como {id, tag}
            l.ids_cantos = l.ids_cantos.map(item => ({
                id: item.id || item,
                tag: item.etiqueta || item.tag || ""
            }));
    ...
    if (idCorto || shareLargo) {
        ...
        try {
            ...
            if (datosCanto && datosCanto.n && datosCanto.i) {
                ...
                // LIMPIEZA AQUÍ: Convertimos a formato {id, tag}
                const listaLimpia = datosCanto.i.map(item => ({
                    id: item.id || item,
...
window.setMomento = (momento) => {
    momentoActivo = momento;
    
    // Filtrado automático de la lista de cantos
    const filtrados = (momento === 'Libre') 
        ? todosLosCantos 
        : todosLosCantos.filter(c => c.moments && c.moments.includes(momento));
    
    renderizarLista(filtrados);
    
    // Feedback visual opcional
    console.log(`Filtrando por: ${momento}`);
};
...
window.toggleCanto = (id) => {
    ...
    if (index !== -1) {
        ...
        if (momentoSeleccionado === 'Libre') {
            // Contamos cuántos elementos NO tienen etiqueta de momento (solo los numéricos)
            const numericos = listaOrdenada.filter(item => !['E', 'P', 'C', 'F'].includes(item.etiqueta));
            etiqueta = (numericos.length + 1).toString();
        } else {
            const mapa = { 'Entrada': 'E', 'Paz': 'P', 'Comunión': 'C', 'Final': 'F' };
            etiqueta = mapa[momentoSeleccionado] || "N";
        }
