...
const initializeCantoPage = (cantoSpecificData, processedCategories) => {
    ...
    // Lógica para aplicar el fondo del canto
    const bodyElement = document.body;
    // Eliminar cualquier clase de fondo existente para evitar duplicados o conflictos
    bodyElement.classList.remove('fondo-catecumenado', 'fondo-precatecumenado', 'fondo-liturgia', 'fondo-eleccion');

    if (cantoSpecificData.fondo) {
        // Añadir la nueva clase de fondo
        bodyElement.classList.add(`fondo-${cantoSpecificData.fondo}`);
        console.log(`Fondo aplicado: fondo-${cantoSpecificData.fondo}`);
    } else {
        console.log("No se especificó un fondo para este canto, usando el fondo por defecto.");
    }
};
