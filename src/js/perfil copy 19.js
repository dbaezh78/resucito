...
// 20.1: APERTURA Y CARGA DE DATOS (LECTURA DESDE SUB-COLECCIÓN HISTORIAL)
window.abrirCalendario = async function(cantoId) {
    ...
    try {
        ...
        // 1. MANEJO DEL MODAL (Evitar que se inhiba la página)
        let modal = document.getElementById('calendar-modal');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'calendar-modal';
            // Añadimos un fondo oscuro para ver que el modal está activo
            modal.style = "position:fixed; top:0; left:0; width:100%; height:100%; z-index:999999; display:flex; align-items:center; justify-content:center; background: rgba(0,0,0,0.5); font-family: sans-serif;";
            document.body.appendChild(modal);
        }
        modal.style.display = "flex";
        // Ponemos un mensaje de carga para que el usuario sepa que está trabajando
        modal.innerHTML = '<div style="background:white; padding:20px; border-radius:10px;">⌛ Cargando historial...</div>';

        fechasHistorialActivas = [];
        fechasOriginalesFull = [];
        totalRegistrosCanto = 0;

        // 2. RUTA DE LA SUB-COLECCIÓN (La que vimos en tu imagen)
        ...
        // IMPORTANTE: Esta función debe estar definida para dibujar el calendario dentro del modal
        if (typeof actualizarVistaCalendario === 'function') {
            actualizarVistaCalendario(cantoId); 
        } else {
            // Si no existe la función, al menos mostramos un botón para cerrar y no bloquear
            modal.innerHTML = `<div style="background:white; padding:20px; border-radius:10px;">
                <p>Registros encontrados: ${totalRegistrosCanto}</p>
                <button onclick="document.getElementById('calendar-modal').style.display='none'">Cerrar</button>
            </div>`;
        }

        document.addEventListener('keydown', manejarEscape);
        console.log(`📅 Historial de ${cantoId} cargado: ${totalRegistrosCanto} registros.`);
