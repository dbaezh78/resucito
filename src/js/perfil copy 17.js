...
// 6: OBTENER FIREBASE: Unificado para leer desde dbdata (Ruta Única)
async function obtenerDatosExtraFirebase(cantoId, uid) {
    try {
        const { getDoc, doc } = await import("https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js");
        
        // Realizamos UNA SOLA llamada al documento unificado
        ...
        if (docSnap.exists()) {
            ...
            // Extraemos los campos directamente
            const datos = {
            ...
            // LOG de depuración para confirmar que lee dbdata
            console.log(`✅ Sección 6 (dbdata) cargada para ${cantoId}:`, datos);

            // 1. Guardar en RAM
            ALMACEN_CANTOS[cantoId] = datos;

            // 2. Guardar en LocalStorage (Escudo)
            localStorage.setItem(`data-${cantoId}`, JSON.stringify(datos));

            // 3. Pintar en la tabla inmediatamente
            if (typeof inyectarDatosEnTabla === 'function') {
                inyectarDatosEnTabla(cantoId, datos, false);
            }
        } else {
            // Si no existe en dbdata, podemos asumir valores por defecto o no hacer nada
            // console.log(`ℹ️ El canto ${cantoId} no tiene datos en dbdata aún.`);
        }
    } catch (e) { 
        console.warn("⚠️ Error en Sección 6 (dbdata):", e); 
    }
}
...
// 21: COMUNICACIÓN ENTRE EQUIPO, NUBE Y RAM (CORREGIDO A DBDATA)
window.sincronizarTodoARam = async function() { 
    ...
    try {
        ...
        if (total === 0) {
            if (texto) texto.innerText = "No se encontraron registros en dbdata.";
        ...
        querySnapshot.forEach((docSnap) => {
            ...
            // 🔍 LOG 1: Ver datos crudos de la nueva colección
            console.log(`📡 LOG 1 > Datos de dbdata [${cantoId}]:`, rawData);

            let fechaFinal = null;
            // En dbdata los campos son directos: fecha, acorde, cejilla
            ...
            // Actualizar tabla visualmente (Sección 5)
            if (typeof inyectarDatosEnTabla === 'function') {
            ...
            if (texto) texto.innerText = `Sincronizando dbdata: ${procesados} de ${total}...`;
        });

        if (texto) texto.innerText = "¡Sincronización dbdata completada!";
        ...
        if (texto) texto.innerText = "Error al conectar con dbdata.";
    }
};
