...
async function renderizarTablaCantos() {
    ...
    try {
        ...
        cantos.forEach(canto => {
            // Buscamos en la RAM (ALMACEN_CANTOS)
            const datosRAM = ALMACEN_CANTOS[canto.id] || null;
            
            // Si hay datos en RAM, los usamos; si no, ponemos valores neutros
            const cejillaVisual = datosRAM ? (datosRAM.cejilla || "0") : "0";
            const numAcorde = datosRAM ? String(datosRAM.acorde) : null;
            
            // Si numAcorde existe en MAPA_ACORDES lo ponemos, si no, ponemos "---"
            const acordeTexto = (numAcorde !== null && MAPA_ACORDES[numAcorde]) 
                                ? MAPA_ACORDES[numAcorde] 
                                : "---";

            const enlaceCanto = `src/index.html?canto=${canto.id}${numAcorde ? '&tonalidad='+numAcorde : ''}${cejillaVisual !== "0" ? '&cejilla='+cejillaVisual : ''}`;
            ...
            html += `
                <tr class="fila-canto" id="fila-${canto.id}">
                    ...
                    <td>${canto.cejilla ?? 0} / <b id="cejilla-tu-${canto.id}" style="color: #bc0009;">${cejillaVisual === "0" ? "-" : cejillaVisual}</b></td>
        ...
        // Llamamos a completar datos para que Firebase sobrescriba los "---"
