...
window.compartirListaLink = (idLista) => {
    ...
    try {
        ...
        navigator.clipboard.writeText(url).then(() => {
            alert("¡Enlace generado con éxito! (Compatible con acentos e iconos)");
        });
    } catch (e) {
...
const detectarLinkCompartido = async () => {
    ...
    if (share) {
        ...
        try {
            ...
            console.log("📦 Datos recibidos del link:", datosCanto);
            
            if (datosCanto.n && datosCanto.i) {
                const idFinal = "imp-" + Date.now();
                // Evitamos duplicar iconos si ya trae la carpeta del archivo
                const nombreLimpio = datosCanto.n.includes("🔗") || datosCanto.n.includes("📂") 
                    ? datosCanto.n 
                    : "🔗 " + datosCanto.n;
            ...
            console.error("❌ Error decodificando el enlace compartido:", e);
            bloqueoSnapshot = false;
            // Limpiamos la URL para que el usuario pueda seguir usando la app
