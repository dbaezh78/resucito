...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        onSnapshot(q, (snapshot) => {
            ...
            // Solo renderizar si el buscador de listas está vacío para no interrumpir al usuario
            const inputBuscaListas = document.getElementById('inputBuscadorListas');
            if (!inputBuscaListas || inputBuscaListas.value === '') {
                renderizarListasUI(listasLocalesCache);
            }
...
window.guardarListaFirebase = async () => {
    ...
    // 2. FORZAR DESPLIEGUE VISUAL INMEDIATO
    const buscadorListas = document.getElementById('inputBuscadorListas');
    if (buscadorListas) buscadorListas.value = ''; // Limpiar búsqueda para que se vea la nueva
    renderizarListasUI(listasLocalesCache); 
...
window.importarLista = (event) => {
    ...
    reader.onload = async (e) => {
        try {
            ...
            // FORZAR DESPLIEGUE TRAS IMPORTACIÓN
...
window.toggleDetalleLista = (idLista) => {
    ...
    if (estaCerrado) {
        ...
        // Soporte para snapshot de Firebase o lista local
    ...
    if (share) {
        try {
            ...
            if (confirm(`¿Importar lista compartida: "${d.n}"?`)) {
                ...
                // FORZAR DESPLIEGUE TRAS COMPARTIDO
                renderizarListasUI(listasLocalesCache);
