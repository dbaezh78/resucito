...
import { 
  ...
  registrarCambioAcorde, 
  registrarCambioTono, 
  registrarValoracionCanto,
  registrarAccionLista, 
  finalizarSesionCantoActual 
} from './bitacoraLogger.js';
window.registrarAccionLista = registrarAccionLista;
...
async function loadSongView(songId) {
  try {
    ...
    // Registrar inicio de sesión en bitácora para el canto
    registrarEntradaCanto(songId, currentCanto.title || currentCanto.tt);
...
function guardarRatingCanto(songId, rating) {
  ...
  const cantoTitulo = (currentCanto && currentCanto.id === songId) ? (currentCanto.title || currentCanto.titulo || songId) : songId;
  registrarValoracionCanto(songId, cantoTitulo, newRating);
...
function saveSingleChordEdit(chosenNote, chosenType) {
  ...
  if (lineChords && lineChords[chordIdx]) {
    ...
    // Registrar cambio de acorde en bitácora
    const chordName = `${lineChords[chordIdx].name}${lineChords[chordIdx].type || ''}`;
    registrarCambioAcorde(songId, currentCanto.title || currentCanto.tt, chordName);
...
function shiftKey(semitones) {
  ...
  if (currentCanto) {
    ...
    const transposedKey = transposeNote(originalSongKey, currentKeyOffset);
    const suffix = originalSongTypeSuffix ? (originalSongTypeSuffix.startsWith(' ') ? '' : ' ') + originalSongTypeSuffix : '';
    const fullCurrentChord = `${transposedKey}${suffix}`;
    const fullOriginalChord = `${originalSongKey}${suffix}`;
    registrarCambioTono(currentCanto.id, currentCanto.title || currentCanto.tt, currentKeyOffset, fullCurrentChord, fullOriginalChord);
  }
}
...
function setupEventListeners() {
  ...
  if (capoSelect) {
    capoSelect.addEventListener('change', () => {
      ...
      updateChordPanel();
      guardarHistorialCanto();
      registrarCambioTraste(currentCanto.id, currentCanto.title || currentCanto.tt, selectedCapo);
    });
  }
  ...
  // --- Autenticación y Cuenta de Usuario (manejado de forma centralizada en ajustes.js) ---
