...
import{c as e,f as t,i as n,k as r,l as i,m as a,p as o,t as s,u as c,v as l,x as u}from"./preload-helper-44F9sIZn.js";import{l as d,u as f}from"./sync-CL2BKT7r.js";var p=[{value:`0`,label:`Precatecumenado`},{value:`1`,label:`Primer Escrutinio`},{value:`1.5`,label:`SHEMA`},{value:`2`,label:`Segundo Escrutinio`},{value:`3`,label:`Iniciación a la Oración`},{value:`4`,label:`Traditio Symboli`},{value:`5`,label:`Redditio Symboli`},{value:`6`,label:`Padre Nuestro`},{value:`7`,label:`Elección`},{value:`8`,label:`Renovación de las Promesas Bautismales`}],m={numero:`Número (N°)`,nombre:`Nombre del Canto`,categoria:`Categoría / Etapa`,acorde:`Acorde`,posacorde:`PosAcorde`,notas:`Notas del Canto`,favorito:`Favorito`,sw:`Caché SW`},h=[],g={},_=new Set,v=new Set(JSON.parse(localStorage.getItem(`mant_hidden_columns`)||`[]`)),y={},b=null,x=new Set;try{let e=localStorage.getItem(`favorites`);e&&(x=new Set(JSON.parse(e)))}catch{}function S(){a(e=>{let t=c();t||n(`page_mantcantos`)||(console.warn(`Acceso no autorizado a Mantenimiento de Cantos. Redirigiendo a Inicio...`),window.location.replace(`./index.html`));let r=document.getElementById(`btn-backup-user-pos`);r&&(r.style.display=t?`inline-flex`:`none`)})}async function C(){try{try{let e=await fetch(`data/chord_positions.json`);e.ok&&(g=await e.json())}catch(e){console.warn(`Error cargando data/chord_positions.json:`,e)}try{_.clear();let e=(await caches.keys()).find(e=>e.startsWith(`resucito-cache-`));e&&(await(await caches.open(e)).keys()).forEach(e=>{let t=new URL(e.url,window.location.href).pathname;t.startsWith(`/`)&&(t=t.substring(1)),_.add(t),_.add(t.split(`?`)[0])})}catch(e){console.warn(`Error leyendo Service Worker Cache:`,e)}try{await f(!0)}catch{}let e=await fetch(`data/songs-index.json`);e.ok&&(h=await e.json()),j(),A()}catch(e){console.error(`Error al cargar datos en mantcantos:`,e);let t=document.getElementById(`mant-table-body`);t&&(t.innerHTML=`<tr><td colspan="8" style="text-align: center; color: #dc3545; padding: 20px;">Error al cargar datos: ${e.message}</td></tr>`)}}function w(e,t){let n=e.id||``;if(t===`numero`)return e.dbno?e.dbno.toString():`S/N`;if(t===`nombre`)return e.title||``;if(t===`categoria`){let e=D(n),t=p.find(t=>t.value===e);return t?t.label:`Precatecumenado`}return t===`acorde`?e.acorde||`-`:t===`posacorde`?g[n]?`Sí`:`No`:t===`notas`?E(n)?`Con notas`:`Sin notas`:t===`favorito`?x.has(n)?`Favorito`:`No favorito`:t===`sw`?T(n)?`En caché`:`Faltante`:``}function T(e){if(!e)return!1;let t=`${e.startsWith(`aet`)?`data/songs-ae`:`data/songs`}/${e}.json`;return _.has(t)}function E(e){let t=localStorage.getItem(`notes_${e}`);return t?t.trim():``}function D(e){return window.globalPositionsCache&&window.globalPositionsCache[e]&&window.globalPositionsCache[e].etapa!==void 0?window.globalPositionsCache[e].etapa.toString():`0`}async function O(e,t){window.globalPositionsCache||(window.globalPositionsCache={}),window.globalPositionsCache[e]||(window.globalPositionsCache[e]={}),window.globalPositionsCache[e].etapa=t,localStorage.setItem(`resucito_global_positions_cache`,JSON.stringify(window.globalPositionsCache));try{await r(u(l,`global_positions`,e),{etapa:t},{merge:!0}),console.log(`Etapa actualizada para ${e} -> ${t}`)}catch(e){console.warn(`Error guardando etapa en Firebase:`,e)}}function k(e,t){t?x.add(e):x.delete(e),localStorage.setItem(`favorites`,JSON.stringify(Array.from(x))),window.favorites&&(t?window.favorites.add(e):window.favorites.delete(e)),typeof window.guardarFavoritosEnNube==`function`&&window.guardarFavoritosEnNube([...x])}function A(){let e=document.getElementById(`mant-table-body`),t=document.getElementById(`mant-count-badge`),n=(document.getElementById(`mant-search-input`)?.value||``).toLowerCase().trim();if(!e)return;let r=h.filter(e=>{let t=e.id||``,r=(e.title||``).toLowerCase(),i=(e.dbno||``).toString().toLowerCase(),a=(e.acorde||``).toLowerCase(),o=E(t).toLowerCase(),s=D(t),c=p.find(e=>e.value===s),l=(c?c.label:``).toLowerCase();if(n&&!(r.includes(n)||i.includes(n)||a.includes(n)||o.includes(n)||l.includes(n)))return!1;for(let[t,n]of Object.entries(y)){if(!n||n.size===0)continue;let r=w(e,t);if(!n.has(r))return!1}return!0});if(t&&(t.innerText=`${r.length} de ${h.length} cantos`),r.length===0){e.innerHTML=`<tr><td colspan="8" style="text-align: center; padding: 30px; color: var(--text-muted);">No se encontraron cantos con los criterios actuales.</td></tr>`;return}e.innerHTML=r.map(e=>{let t=e.id,n=D(t),r=!!g[t],i=E(t),a=x.has(t),o=T(t),s=p.map(e=>`
        ...
        `}).join(``),e.querySelectorAll(`.stage-select`).forEach(e=>{e.addEventListener(`change`,async t=>{let n=e.dataset.songId,r=t.target.value;await O(n,r)})}),e.querySelectorAll(`.fav-checkbox`).forEach(e=>{e.addEventListener(`change`,t=>{let n=e.dataset.songId;k(n,t.target.checked)})}),j()}function j(){let e=[`numero`,`nombre`,`categoria`,`acorde`,`posacorde`,`notas`,`favorito`,`sw`];e.forEach(e=>{let t=v.has(e),n=document.querySelector(`th[data-col="${e}"]`);n&&(n.style.display=t?`none`:``),document.querySelectorAll(`td[data-col="${e}"]`).forEach(e=>e.style.display=t?`none`:``)}),e.forEach(e=>{let t=document.querySelector(`.filter-indicator[data-col-indicator="${e}"]`);t&&(t.style.display=y[e]?`inline-block`:`none`)});let t=document.getElementById(`hidden-columns-bar`),n=document.getElementById(`hidden-columns-list`);t&&n&&(v.size>0?(n.innerText=Array.from(v).map(e=>m[e]||e).join(`, `),t.style.display=`flex`):t.style.display=`none`),localStorage.setItem(`mant_hidden_columns`,JSON.stringify(Array.from(v)))}function M(e){e&&(v.add(e),j())}function N(){v.clear(),j()}var P=document.getElementById(`col-context-menu`),F=document.getElementById(`mant-table-header-row`);F&&F.addEventListener(`contextmenu`,e=>{let t=e.target.closest(`th`);if(!t||(e.preventDefault(),b=t.dataset.col,!b))return;let n=Math.min(e.clientX,window.innerWidth-210),r=Math.min(e.clientY,window.innerHeight-150);P.style.left=`${n}px`,P.style.top=`${r}px`,P.style.display=`block`}),document.addEventListener(`click`,e=>{P&&!P.contains(e.target)&&(P.style.display=`none`)}),document.getElementById(`ctx-hide-col`)?.addEventListener(`click`,()=>{b&&M(b),P.style.display=`none`}),document.getElementById(`ctx-show-all-cols`)?.addEventListener(`click`,()=>{N(),P.style.display=`none`}),document.getElementById(`btn-restore-columns`)?.addEventListener(`click`,()=>{N()});var I=document.getElementById(`modal-filter-col`),L=document.getElementById(`modal-filter-title`),R=document.getElementById(`modal-filter-desc`),z=document.getElementById(`modal-filter-search`),B=document.getElementById(`modal-filter-options-list`),V=document.getElementById(`modal-filter-selected-count`),H=document.getElementById(`modal-filter-select-all`),U=document.getElementById(`modal-filter-deselect-all`),W=document.getElementById(`modal-filter-close`),G=document.getElementById(`modal-filter-btn-apply`),K=document.getElementById(`modal-filter-btn-clear`),q=[],J=new Set;function Y(){V&&(V.innerText=`${J.size} de ${q.length} seleccionados`)}function X(e=``){if(!B)return;let t=e.toLowerCase().trim(),n=q.filter(e=>!t||e.val.toLowerCase().includes(t));if(n.length===0){B.innerHTML=`<div style="padding: 16px; text-align: center; color: var(--text-muted); font-size: 0.82rem;">No hay opciones que coincidan</div>`;return}B.innerHTML=n.map(e=>{let t=J.has(e.val),n=e.val.replace(/"/g,`&quot;`);return`
        ...
        `}).join(``),B.querySelectorAll(`.filter-option-checkbox`).forEach(e=>{e.addEventListener(`change`,t=>{let n=e.dataset.val;e.checked?J.add(n):J.delete(n),Y()})})}document.getElementById(`ctx-filter-col`)?.addEventListener(`click`,()=>{if(P.style.display=`none`,!b)return;let e=m[b]||b;L.innerHTML=`<span class="material-symbols-outlined" style="color: var(--accent-color, #d01212);">filter_alt</span> Filtrar por ${e}`,R.innerText=`Selecciona los valores de "${e}" que deseas visualizar en la tabla:`,z&&(z.value=``);let t=new Map;h.forEach(e=>{let n=w(e,b);t.set(n,(t.get(n)||0)+1)}),q=Array.from(t.entries()).map(([e,t])=>({val:e,count:t})),q.sort((e,t)=>{let n=parseInt(e.val.replace(/\D/g,``),10),r=parseInt(t.val.replace(/\D/g,``),10);return!isNaN(n)&&!isNaN(r)&&b===`numero`?n-r:e.val.localeCompare(t.val,`es`,{numeric:!0})}),J=y[b]&&y[b].size>0?new Set(y[b]):new Set(q.map(e=>e.val)),X(),Y(),I.style.display=`flex`,setTimeout(()=>z?.focus(),50)}),z?.addEventListener(`input`,()=>{X(z.value)}),H?.addEventListener(`click`,()=>{q.forEach(e=>J.add(e.val)),X(z?z.value:``),Y()}),U?.addEventListener(`click`,()=>{J.clear(),X(z?z.value:``),Y()}),W?.addEventListener(`click`,()=>{I.style.display=`none`}),K?.addEventListener(`click`,()=>{b&&delete y[b],I.style.display=`none`,A()}),G?.addEventListener(`click`,()=>{b&&(J.size===q.length?delete y[b]:y[b]=new Set(J)),I.style.display=`none`,A()});var Z=document.getElementById(`mant-search-input`),Q=document.getElementById(`mant-search-clear`);Z?.addEventListener(`input`,()=>{Q&&(Q.style.display=Z.value?`block`:`none`),A()}),Q?.addEventListener(`click`,()=>{Z.value=``,Q.style.display=`none`,A()}),document.getElementById(`btn-refresh-data`)?.addEventListener(`click`,()=>{C()}),document.getElementById(`btn-backup-user-pos`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`btn-backup-user-pos`);try{e.disabled=!0,e.innerHTML=`<span class="material-symbols-outlined">sync</span> Respaldando...`;let t=await d(),n=`✅ Respaldo de Posiciones de Usuario completado:
...
📥 Se ha descargado también el archivo en tu navegador.`,alert(n)}catch(e){console.error(`Error al respaldar posiciones:`,e),alert(`Error al respaldar: `+e.message)}finally{e.disabled=!1,e.innerHTML=`<span class="material-symbols-outlined" style="color: var(--accent-color, #d01212);">save_as</span> Respaldar Posiciones`}}),S(),C(),(function(){if(document.getElementById(`nav-wrapper`))return;window.addEventListener(`beforeinstallprompt`,e=>{e.preventDefault(),window.deferredPrompt=e,console.log(`📥 PWA: beforeinstallprompt guardado.`);let t=document.getElementById(`installButton`);t&&(t.style.opacity=`1`,t.style.pointerEvents=`auto`)}),window.addEventListener(`appinstalled`,e=>{console.log(`🎉 PWA: La aplicación fue instalada con éxito.`),window.deferredPrompt=null;let t=document.getElementById(`installButton`);t&&(t.style.opacity=`0.5`,t.style.pointerEvents=`none`)});let r=window.APP_VERSION||localStorage.getItem(`resucito_installed_version`)||`2.1.00`,l=`
    <div id="nav-wrapper">
      ...
      <div class="settings-modal-content" style="max-width: 480px; width: 92%; padding: 20px; border-radius: 20px;">
        ...
        <div class="settings-body" style="padding: 4px; color: var(--text-color);">
          <div style="text-align: center; margin-bottom: 20px;">
            ...
            <span style="background: var(--accent-color, #d01212); color: #fff; padding: 3px 12px; border-radius: 12px; font-size: 0.78rem; font-weight: 600; display: inline-block; margin-top: 4px;">Versión v${r}</span>
          </div>
          ...
          <div class="version-log-item" style="margin-bottom: 16px; background: rgba(0,0,0,0.03); padding: 12px; border-radius: 12px; border: 1px solid var(--panel-border);">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
              <strong style="color: var(--accent-color, #d01212); font-size: 0.95rem;">v${r} (Versión Actual)</strong>
  ...
  `),f()};document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,d):d();function f(){let l=document.getElementById(`nav-toggle`);l&&l.addEventListener(`click`,p);let u=e=>{e.preventDefault(),document.querySelectorAll(`.nav-submenu`).forEach(e=>e.classList.remove(`active`));let t=document.getElementById(`account-popup-card`);t&&t.classList.add(`hidden`);let n=window.location.pathname.includes(`/src/`);if(window.location.pathname.includes(`perfil.html`)||window.location.pathname.includes(`chat.html`)||!document.getElementById(`dashboard-view`)){window.location.href=n?`../index.html`:`./index.html`;return}let r=document.getElementById(`dashboard-view`),i=document.getElementById(`song-viewer-view`);r&&i&&(i.style.display=`none`,r.style.display=`block`,window.location.hash=``,window.scrollTo({top:0,behavior:`smooth`}))},d=document.getElementById(`btn-nav-inicio`);d&&d.addEventListener(`click`,u);let f=document.getElementById(`nav-resucito-camino`);f&&f.addEventListener(`click`,u);let m=(e,t)=>{let n=document.getElementById(e),r=document.getElementById(t);n&&r&&n.addEventListener(`click`,e=>{e.stopPropagation();let t=document.getElementById(`account-popup-card`);t&&t.classList.add(`hidden`),document.querySelectorAll(`.nav-submenu`).forEach(e=>{e!==r&&e.classList.remove(`active`)}),r.classList.toggle(`active`)})};m(`btn-nav-menu`,`nav-submenu`),m(`btn-nav-neocate`,`nav-submenu-neocate`),m(`btn-nav-resucito`,`nav-submenu-resucito`);let h=document.getElementById(`btn-open-settings`);h&&h.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation();let t=document.getElementById(`account-popup-card`);t&&t.classList.add(`hidden`),typeof window.abrirModalConfiguracion==`function`?window.abrirModalConfiguracion():s(()=>import(`./ajustes-D6HqGDky.js`).then(()=>{if(typeof window.abrirModalConfiguracion==`function`)window.abrirModalConfiguracion();else{let e=document.getElementById(`settings-modal`);e&&(e.style.display=`flex`)}}),__vite__mapDeps([0,1]),import.meta.url).catch(e=>{console.warn(`No se pudo cargar ajustes in-situ:`,e),window.location.href=`./index.html#ajustes`})});let _=document.getElementById(`account-popup-card`),v=document.getElementById(`account-popup-close`),y=document.getElementById(`account-popup-toggle-header`),b=document.getElementById(`account-actions-list`),x=document.getElementById(`account-toggle-text`),S=document.getElementById(`account-toggle-icon`);v&&_&&v.addEventListener(`click`,e=>{e.stopPropagation(),_.classList.add(`hidden`)}),y&&b&&x&&S&&y.addEventListener(`click`,e=>{e.stopPropagation(),b.classList.contains(`collapsed`)?(b.classList.remove(`collapsed`),x.innerText=`Ocultar`,S.innerText=`expand_less`):(b.classList.add(`collapsed`),x.innerText=`Mostrar`,S.innerText=`expand_more`)});let C=document.getElementById(`account-popup-manage`),w=document.getElementById(`account-action-perfil`),T=document.getElementById(`account-action-preparar`),E=document.getElementById(`account-action-actualizar`),D=document.getElementById(`account-action-logout`),O=document.getElementById(`account-info-app-link`),k=document.getElementById(`app-info-modal`),A=document.getElementById(`close-app-info-modal`),j=window.location.pathname.includes(`/src/`),M=e=>{e.stopPropagation(),window.location.href=j?`../perfil.html`:`perfil.html`};C&&C.addEventListener(`click`,M),w&&w.addEventListener(`click`,M),T&&T.addEventListener(`click`,e=>{e.stopPropagation(),window.location.href=j?`../preparar.html`:`preparar.html`});let N=document.getElementById(`account-action-bitacora`);N&&N.addEventListener(`click`,e=>{e.stopPropagation(),window.location.href=j?`../bitacora.html`:`bitacora.html`}),E&&E.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),F()});let P=document.getElementById(`account-action-chat`);P&&P.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),_&&_.classList.add(`hidden`),window.location.href=j?`../chat.html`:`chat.html`}),D&&D.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),_&&_.classList.add(`hidden`),window.mostrarConfirmacion({titulo:`Cerrar Sesión`,mensaje:`¿Desea cerrar sesión de su cuenta?`,icono:`logout`,textoSi:`Sí`,textoNo:`No`,onConfirm:async()=>{window.firebaseAPI?.logout?await window.firebaseAPI.logout():o()}})}),O&&k&&O.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),_&&_.classList.add(`hidden`),k.style.display=`flex`}),A&&k&&A.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),k.style.display=`none`});let F=()=>{if(!navigator.onLine){window.mostrarAlerta?window.mostrarAlerta({titulo:`Sin Conexión`,mensaje:`No puede Actualizar sin internet`,icono:`wifi_off`}):alert(`⚠️ No puede Actualizar sin internet`);return}_&&_.classList.add(`hidden`),k&&(k.style.display=`none`);let e=window._latestRemoteVersion||`Nueva versión`,t=r||`2.0`;window.mostrarConfirmacion({titulo:`Actualizar Aplicación`,mensaje:`¿Desea actualizar de la versión v${t} a la v${e}? Sus datos personales y cantos se conservarán intactos.`,icono:`system_update`,textoSi:`Sí, Actualizar`,textoNo:`Cancelar`,onConfirm:async()=>{let n=[`version.json`,`sw.js`,`index.html`,`src/main.js`,`src/navegador.js`,`src/navegador.css`,`src/style.css`,`data/songs-index.json`,`data/ajustes_modal.html`],r=0,i=n.length;window.mostrarProgreso({titulo:`Actualizando App`,mensaje:`Comparando v${t} ➔ v${e}\nIniciando descarga de archivos...`,icono:`sync`,porcentaje:5});for(let e of n){try{await fetch(e+`?t=`+Date.now(),{cache:`reload`})}catch(t){console.warn(`Aviso al descargar ${e}:`,t)}r++;let t=Math.round(r/i*40);window.mostrarProgreso({titulo:`Actualizando Sistema`,mensaje:`Descargando: ${e} (${r}/${i})`,icono:`download`,porcentaje:t}),await new Promise(e=>setTimeout(e,60))}try{if(window.mostrarProgreso({titulo:`Sincronizando Todo el Cancionero`,mensaje:`Analizando y descargando todos los recursos faltantes...`,icono:`cloud_sync`,porcentaje:35}),typeof window.cargarTodosLosRecursosFaltantes==`function`)await window.cargarTodosLosRecursosFaltantes(e=>{let t=35+Math.round(e.percent/100*60);window.mostrarProgreso({titulo:`Descargando Recursos Faltantes`,mensaje:`${e.status||``} (${e.current||0}/${e.total||0})`,icono:`download`,porcentaje:t})});else{let e=(await caches.keys()).find(e=>e.startsWith(`resucito-cache-`))||`resucito-cache-v311`,t=await caches.open(e),n=await fetch(`data/songs-index.json?t=`+Date.now());if(n.ok){let e=await n.clone().json();await t.put(`data/songs-index.json`,n);for(let n=0;n<e.length;n+=10){let r=e.slice(n,n+10);await Promise.all(r.map(async e=>{let n=`${e.id&&e.id.startsWith(`aet`)?`data/songs-ae`:`data/songs`}/${e.id}.json?offline=true`;try{let e=await fetch(n);e.ok&&await t.put(n,e)}catch{}}))}}}}catch(e){console.warn(`Aviso en fase de sincronización de recursos:`,e)}if(`serviceWorker`in navigator)try{let e=await navigator.serviceWorker.getRegistration();e&&(e.waiting&&e.waiting.postMessage({type:`SKIP_WAITING`}),await e.update())}catch{}window._latestRemoteVersion&&localStorage.setItem(`resucito_installed_version`,window._latestRemoteVersion),window.mostrarProgreso({titulo:`¡Actualización Lista!`,mensaje:`Todo el contenido y la versión v${e} están listos. Reiniciando...`,icono:`check_circle`,porcentaje:100}),setTimeout(()=>{window.location.reload()},900)}})};function I(e,t){if(!e||!t)return!1;let n=String(e).replace(/^v/i,``).split(`.`).map(e=>parseInt(e,10)||0),r=String(t).replace(/^v/i,``).split(`.`).map(e=>parseInt(e,10)||0),i=Math.max(n.length,r.length);for(let e=0;e<i;e++){let t=n[e]||0,i=r[e]||0;if(t>i)return!0;if(t<i)return!1}return!1}async function L(){try{let e=(window.location.origin||``)+`/version.json?t=`+Date.now();console.log(`🔍 Comprobando versión remota en:`,e);let t=await fetch(e,{cache:`no-store`});if(!t.ok){console.warn(`⚠️ No se pudo obtener version.json, status:`,t.status);return}let n=await t.json();if(console.log(`📦 Info de versión recibida:`,n,`Versión local instalada:`,r),n&&n.latestVersion&&I(n.latestVersion,r)){console.log(`✨ ¡Nueva versión detectada!: v${n.latestVersion} (Actual: v${r})`),window._latestRemoteVersion=n.latestVersion,E&&(E.classList.add(`has-update-ready`),E.innerHTML=`
            ...
            `);let e=document.querySelector(`#app-info-modal .settings-body`);if(e&&!document.getElementById(`app-update-live-card`)){let t=document.createElement(`div`);t.id=`app-update-live-card`,t.className=`app-update-badge-container`,t.style.cssText=`margin-bottom: 20px; background: rgba(0,0,0,0.04); padding: 16px; border-radius: 18px; border: 1px solid rgba(255, 215, 0, 0.4);`,t.innerHTML=`
              ...
              <div style="font-size: 0.78rem; color: var(--text-muted); margin-top: 6px;">
                Versión actual: v${r} ➔ Nueva: v${n.latestVersion}
              </div>
            `;let i=e.firstElementChild;i?i.insertAdjacentElement(`afterend`,t):e.prepend(t),document.getElementById(`btn-ring-update-modal`)?.addEventListener(`click`,F),document.getElementById(`btn-banner-update-modal`)?.addEventListener(`click`,F)}}}catch(e){console.warn(`No se pudo verificar actualización remota:`,e)}}setTimeout(L,1500),document.addEventListener(`click`,e=>{document.querySelectorAll(`.nav-submenu`).forEach(e=>e.classList.remove(`active`));let t=document.getElementById(`nav-google-auth`),n=document.getElementById(`custom-confirm-modal`);_&&!_.contains(e.target)&&(!t||!t.contains(e.target))&&_.classList.add(`hidden`),k&&e.target===k&&(k.style.display=`none`),n&&e.target===n&&(n.style.display=`none`)});let R=e=>{let n=document.getElementById(`nav-auth-icon`),r=document.getElementById(`nav-auth-text`),i=document.getElementById(`nav-google-auth`),a=document.getElementById(`account-popup-card`),o=document.getElementById(`account-popup-email`),s=document.getElementById(`account-popup-greeting`),c=document.getElementById(`account-popup-img`);!i||!n||!r||(e?(o&&(o.innerText=e.email||`usuario@gmail.com`),s&&(s.innerText=`¡Hola, ${e.displayName||`Usuario`}!`),c&&e.photoURL&&(c.src=e.photoURL),n.innerHTML=e.photoURL?`<img src="${e.photoURL}" class="dbperfil">`:`<span class="material-symbols-outlined arrow-icon">person</span>`,r.innerText=`Cuenta`,i.onclick=e=>{e.preventDefault(),e.stopPropagation(),document.querySelectorAll(`.nav-submenu`).forEach(e=>e.classList.remove(`active`)),a&&a.classList.toggle(`hidden`)}):(n.innerHTML=`<span class="material-symbols-outlined arrow-icon">account_circle</span>`,r.innerText=`Entrar`,a&&a.classList.add(`hidden`),i.onclick=e=>{e.preventDefault(),e.stopPropagation();let n=window.firebaseAPI?.getCurrentUser?.();if(n){R(n),a&&a.classList.remove(`hidden`);return}window.firebaseAPI?.login?window.firebaseAPI.login():t()}))};function z(){let t=c(),r=t||n(`page_inicio`),i=t||n(`page_perfil`),a=t||n(`page_preparar`),o=t||n(`page_bitacora`),s=t||n(`page_introduccion`),l=t||n(`page_resucito_pdf`),u=t||n(`page_instalar_app`),d=t||n(`page_mantcantos`),f=t||n(`page_respaldo`),p=e()||window.firebaseAPI?.getCurrentUser?.(),m=t||!!p&&n(`page_chat`),h=document.getElementById(`btn-nav-inicio`);h&&(h.style.display=r?`flex`:`none`);let g=document.getElementById(`nav-resucito-camino`),_=document.getElementById(`nav-resucito-perfil`),v=document.getElementById(`nav-resucito-preparar`),y=document.getElementById(`nav-resucito-bitacora`),b=document.getElementById(`nav-resucito-intro`),x=document.getElementById(`nav-resucito-pdf`),S=document.getElementById(`nav-resucito-mantcantos`),C=document.getElementById(`nav-resucito-respaldo`),w=document.getElementById(`nav-resucito-chat`),T=document.getElementById(`installButton`);g&&(g.style.display=r?`flex`:`none`),_&&(_.style.display=i?`flex`:`none`),v&&(v.style.display=a?`flex`:`none`),y&&(y.style.display=o?`flex`:`none`),b&&(b.style.display=s?`flex`:`none`),x&&(x.style.display=l?`flex`:`none`),S&&(S.style.display=d?`flex`:`none`),C&&(C.style.display=f?`flex`:`none`),w&&(w.style.display=m?`flex`:`none`),T&&(T.style.display=u?`flex`:`none`);let E=document.getElementById(`account-action-preparar`),D=document.getElementById(`account-action-perfil`),O=document.getElementById(`account-action-bitacora`),k=document.getElementById(`account-action-chat`),A=document.getElementById(`account-popup-manage`);E&&(E.style.display=a?`flex`:`none`),D&&(D.style.display=i?`flex`:`none`),O&&(O.style.display=o?`flex`:`none`),k&&(k.style.display=m?`flex`:`none`),A&&(A.style.display=i?`block`:`none`),B()}function B(){if(!i())return;let t=window.location.pathname.toLowerCase();c()||(t.includes(`perfil.html`)&&!n(`page_perfil`)?(console.warn(`Acceso denegado a perfil.html por permisos. Redirigiendo a Inicio...`),window.location.replace(`./index.html`)):t.includes(`preparar.html`)&&!n(`page_preparar`)?(console.warn(`Acceso denegado a preparar.html por permisos. Redirigiendo a Inicio...`),window.location.replace(`./index.html`)):t.includes(`bitacora.html`)&&!n(`page_bitacora`)?(console.warn(`Acceso denegado a bitacora.html por permisos. Redirigiendo a Inicio...`),window.location.replace(`./index.html`)):t.includes(`intro.html`)&&!n(`page_introduccion`)?(console.warn(`Acceso denegado a intro.html por permisos. Redirigiendo a Inicio...`),window.location.replace(`/index.html`)):t.includes(`mantcantos.html`)&&!n(`page_mantcantos`)?(console.warn(`Acceso denegado a mantcantos.html por permisos. Redirigiendo a Inicio...`),window.location.replace(`./index.html`)):t.includes(`respaldo.html`)&&!n(`page_respaldo`)?(console.warn(`Acceso denegado a respaldo.html por permisos. Redirigiendo a Inicio...`),window.location.replace(`./index.html`)):t.includes(`firebase.html`)&&!n(`page_firebase`)?(console.warn(`Acceso denegado a firebase.html por permisos. Redirigiendo a Inicio...`),window.location.replace(`./index.html`)):t.includes(`chat.html`)&&(!(e()||window.firebaseAPI?.getCurrentUser?.())||!n(`page_chat`))&&console.warn(`Acceso denegado a chat.html para usuarios no autenticados o sin permisos.`))}window.updateNavPagesVisibility=z,window.checkCurrentPagePermissionAndRedirect=B,z(),a(e=>{R(e),z()});let V=document.getElementById(`installButton`);V&&(window.deferredPrompt||(V.style.opacity=`0.85`),V.addEventListener(`click`,async e=>{e.preventDefault(),e.stopPropagation(),document.querySelectorAll(`.nav-submenu`).forEach(e=>e.classList.remove(`active`));let t=window.deferredPrompt;if(t){t.prompt();let{outcome:e}=await t.userChoice;console.log(`PWA: Elección del usuario para instalar: ${e}`),window.deferredPrompt=null,e===`accepted`&&(V.style.opacity=`0.5`,V.style.pointerEvents=`none`)}else window.mostrarAlerta?window.mostrarAlerta({titulo:`Instalar Aplicación`,mensaje:`Si no ves la ventana de instalación, puedes instalarla manualmente desde el menú de opciones de tu navegador seleccionando "Instalar aplicación" o "Agregar a la pantalla de inicio" (en iPhone usa la opción "Compartir" > "Agregar a pantalla de inicio").`,icono:`download_for_offline`}):alert(`Para instalar la aplicación, abre el menú de tu navegador y selecciona "Instalar aplicación" o "Agregar a la pantalla de inicio" (en iPhone, presiona el botón "Compartir" y luego "Agregar a pantalla de inicio").`)})),j&&document.querySelectorAll(`#nav-submenu-resucito a, .account-popup-footer a`).forEach(e=>{let t=e.getAttribute(`href`);t&&!t.startsWith(`http`)&&!t.startsWith(`#`)&&!t.startsWith(`/`)&&!t.startsWith(`../`)&&(t.startsWith(`src/`)?e.setAttribute(`href`,t.replace(`src/`,``)):e.setAttribute(`href`,`../`+t))}),g()}function p(){let e=document.getElementById(`nav-wrapper`),t=document.getElementById(`toggle-icon`);e&&e.classList.toggle(`hidden`),t&&t.classList.toggle(`rotate-180`)}window.toggleNavbar=p;let m;function h(){if(localStorage.getItem(`pref-autohide-nav`)!==`true`){m&&clearTimeout(m);return}clearTimeout(m),m=setTimeout(()=>{let e=document.getElementById(`nav-wrapper`);e&&!e.classList.contains(`hidden`)&&window.toggleNavbar()},3e4)}window.startAutoHideTimer=h,document.addEventListener(`mousemove`,h),document.addEventListener(`touchstart`,h),document.addEventListener(`scroll`,h);function g(){let e=localStorage.getItem(`nav-color-text`),t=localStorage.getItem(`nav-color-text-hover`),n=localStorage.getItem(`nav-color-bg`),r=localStorage.getItem(`nav-color-bg-hover`),i=localStorage.getItem(`nav-color-btn-bg`),a=localStorage.getItem(`nav-color-btn-bg-hover`)||localStorage.getItem(`nav-color-btn-hover-bg`),o=localStorage.getItem(`nav-color-icon`),s=localStorage.getItem(`nav-color-icon-hover`),c=localStorage.getItem(`nav-color-submenu-icon`),l=localStorage.getItem(`nav-color-submenu-icon-hover`),u=localStorage.getItem(`nav-color-wrapper-bg`),d=localStorage.getItem(`nav-color-wrapper-bg-hover`)||localStorage.getItem(`nav-color-wrapper-hover-bg`),f=document.documentElement;e?f.style.setProperty(`--nav-text-color`,e):f.style.removeProperty(`--nav-text-color`),t?f.style.setProperty(`--nav-text-hover-color`,t):f.style.removeProperty(`--nav-text-hover-color`),n?f.style.setProperty(`--nav-bg-color`,n):f.style.removeProperty(`--nav-bg-color`),r?f.style.setProperty(`--nav-bg-hover-color`,r):f.style.removeProperty(`--nav-bg-hover-color`),i?f.style.setProperty(`--nav-btn-bg`,i):f.style.removeProperty(`--nav-btn-bg`),a?f.style.setProperty(`--nav-btn-hover-bg`,a):f.style.removeProperty(`--nav-btn-hover-bg`),o?f.style.setProperty(`--nav-icon-color`,o):f.style.removeProperty(`--nav-icon-color`),s?f.style.setProperty(`--nav-icon-hover-color`,s):f.style.removeProperty(`--nav-icon-hover-color`),c?f.style.setProperty(`--nav-submenu-icon-color`,c):f.style.removeProperty(`--nav-submenu-icon-color`),l?f.style.setProperty(`--nav-submenu-icon-hover-color`,l):f.style.removeProperty(`--nav-submenu-icon-hover-color`),u?f.style.setProperty(`--nav-wrapper-bg`,u):f.style.removeProperty(`--nav-wrapper-bg`),d?f.style.setProperty(`--nav-wrapper-hover-bg`,d):f.style.removeProperty(`--nav-wrapper-hover-bg`)}window.applyNavTheme=g})();