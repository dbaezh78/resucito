...
import{O as e,_ as t,b as n,d as r,f as i,i as a,l as o,p as s,t as c,u as l}from"./preload-helper-PeQqBfsS.js";import{l as u,u as d}from"./sync-D_Aow4CG.js";var f=[{value:`0`,label:`Precatecumenado`},{value:`1`,label:`Primer Escrutinio`},{value:`1.5`,label:`SHEMA`},{value:`2`,label:`Segundo Escrutinio`},{value:`3`,label:`Iniciación a la Oración`},{value:`4`,label:`Traditio Symboli`},{value:`5`,label:`Redditio Symboli`},{value:`6`,label:`Padre Nuestro`},{value:`7`,label:`Elección`},{value:`8`,label:`Renovación de las Promesas Bautismales`}],p={numero:`Número (N°)`,nombre:`Nombre del Canto`,categoria:`Categoría / Etapa`,acorde:`Acorde`,posacorde:`PosAcorde`,notas:`Notas del Canto`,favorito:`Favorito`,sw:`Caché SW`},m=[],h={},g=new Set,_=new Set(JSON.parse(localStorage.getItem(`mant_hidden_columns`)||`[]`)),v={},y=null,b=new Set;try{let e=localStorage.getItem(`favorites`);e&&(b=new Set(JSON.parse(e)))}catch{}function x(){s(e=>{let t=l();t||a(`page_mantcantos`)||(console.warn(`Acceso no autorizado a Mantenimiento de Cantos. Redirigiendo a Inicio...`),window.location.replace(`./index.html`));let n=document.getElementById(`btn-backup-user-pos`);n&&(n.style.display=t?`inline-flex`:`none`)})}async function S(){try{try{let e=await fetch(`data/chord_positions.json`);e.ok&&(h=await e.json())}catch(e){console.warn(`Error cargando data/chord_positions.json:`,e)}try{g.clear();let e=(await caches.keys()).find(e=>e.startsWith(`resucito-cache-`));e&&(await(await caches.open(e)).keys()).forEach(e=>{let t=new URL(e.url,window.location.href).pathname;t.startsWith(`/`)&&(t=t.substring(1)),g.add(t),g.add(t.split(`?`)[0])})}catch(e){console.warn(`Error leyendo Service Worker Cache:`,e)}try{await d(!0)}catch{}let e=await fetch(`data/songs-index.json`);e.ok&&(m=await e.json()),A(),k()}catch(e){console.error(`Error al cargar datos en mantcantos:`,e);let t=document.getElementById(`mant-table-body`);t&&(t.innerHTML=`<tr><td colspan="8" style="text-align: center; color: #dc3545; padding: 20px;">Error al cargar datos: ${e.message}</td></tr>`)}}function C(e,t){let n=e.id||``;if(t===`numero`)return e.dbno?e.dbno.toString():`S/N`;if(t===`nombre`)return e.title||``;if(t===`categoria`){let e=E(n),t=f.find(t=>t.value===e);return t?t.label:`Precatecumenado`}return t===`acorde`?e.acorde||`-`:t===`posacorde`?h[n]?`Sí`:`No`:t===`notas`?T(n)?`Con notas`:`Sin notas`:t===`favorito`?b.has(n)?`Favorito`:`No favorito`:t===`sw`?w(n)?`En caché`:`Faltante`:``}function w(e){if(!e)return!1;let t=`${e.startsWith(`aet`)?`data/songs-ae`:`data/songs`}/${e}.json`;return g.has(t)}function T(e){let t=localStorage.getItem(`notes_${e}`);return t?t.trim():``}function E(e){return window.globalPositionsCache&&window.globalPositionsCache[e]&&window.globalPositionsCache[e].etapa!==void 0?window.globalPositionsCache[e].etapa.toString():`0`}async function D(r,i){window.globalPositionsCache||(window.globalPositionsCache={}),window.globalPositionsCache[r]||(window.globalPositionsCache[r]={}),window.globalPositionsCache[r].etapa=i,localStorage.setItem(`resucito_global_positions_cache`,JSON.stringify(window.globalPositionsCache));try{await e(n(t,`global_positions`,r),{etapa:i},{merge:!0}),console.log(`Etapa actualizada para ${r} -> ${i}`)}catch(e){console.warn(`Error guardando etapa en Firebase:`,e)}}function O(e,t){t?b.add(e):b.delete(e),localStorage.setItem(`favorites`,JSON.stringify(Array.from(b))),window.favorites&&(t?window.favorites.add(e):window.favorites.delete(e)),typeof window.guardarFavoritosEnNube==`function`&&window.guardarFavoritosEnNube([...b])}function k(){let e=document.getElementById(`mant-table-body`),t=document.getElementById(`mant-count-badge`),n=(document.getElementById(`mant-search-input`)?.value||``).toLowerCase().trim();if(!e)return;let r=m.filter(e=>{let t=e.id||``,r=(e.title||``).toLowerCase(),i=(e.dbno||``).toString().toLowerCase(),a=(e.acorde||``).toLowerCase(),o=T(t).toLowerCase(),s=E(t),c=f.find(e=>e.value===s),l=(c?c.label:``).toLowerCase();if(n&&!(r.includes(n)||i.includes(n)||a.includes(n)||o.includes(n)||l.includes(n)))return!1;for(let[t,n]of Object.entries(v)){if(!n||n.size===0)continue;let r=C(e,t);if(!n.has(r))return!1}return!0});if(t&&(t.innerText=`${r.length} de ${m.length} cantos`),r.length===0){e.innerHTML=`<tr><td colspan="8" style="text-align: center; padding: 30px; color: var(--text-muted);">No se encontraron cantos con los criterios actuales.</td></tr>`;return}e.innerHTML=r.map(e=>{let t=e.id,n=E(t),r=!!h[t],i=T(t),a=b.has(t),o=w(t),s=f.map(e=>`
        ...
        `}).join(``),e.querySelectorAll(`.stage-select`).forEach(e=>{e.addEventListener(`change`,async t=>{let n=e.dataset.songId,r=t.target.value;await D(n,r)})}),e.querySelectorAll(`.fav-checkbox`).forEach(e=>{e.addEventListener(`change`,t=>{let n=e.dataset.songId;O(n,t.target.checked)})}),A()}function A(){let e=[`numero`,`nombre`,`categoria`,`acorde`,`posacorde`,`notas`,`favorito`,`sw`];e.forEach(e=>{let t=_.has(e),n=document.querySelector(`th[data-col="${e}"]`);n&&(n.style.display=t?`none`:``),document.querySelectorAll(`td[data-col="${e}"]`).forEach(e=>e.style.display=t?`none`:``)}),e.forEach(e=>{let t=document.querySelector(`.filter-indicator[data-col-indicator="${e}"]`);t&&(t.style.display=v[e]?`inline-block`:`none`)});let t=document.getElementById(`hidden-columns-bar`),n=document.getElementById(`hidden-columns-list`);t&&n&&(_.size>0?(n.innerText=Array.from(_).map(e=>p[e]||e).join(`, `),t.style.display=`flex`):t.style.display=`none`),localStorage.setItem(`mant_hidden_columns`,JSON.stringify(Array.from(_)))}function j(e){e&&(_.add(e),A())}function M(){_.clear(),A()}var N=document.getElementById(`col-context-menu`),P=document.getElementById(`mant-table-header-row`);P&&P.addEventListener(`contextmenu`,e=>{let t=e.target.closest(`th`);if(!t||(e.preventDefault(),y=t.dataset.col,!y))return;let n=Math.min(e.clientX,window.innerWidth-210),r=Math.min(e.clientY,window.innerHeight-150);N.style.left=`${n}px`,N.style.top=`${r}px`,N.style.display=`block`}),document.addEventListener(`click`,e=>{N&&!N.contains(e.target)&&(N.style.display=`none`)}),document.getElementById(`ctx-hide-col`)?.addEventListener(`click`,()=>{y&&j(y),N.style.display=`none`}),document.getElementById(`ctx-show-all-cols`)?.addEventListener(`click`,()=>{M(),N.style.display=`none`}),document.getElementById(`btn-restore-columns`)?.addEventListener(`click`,()=>{M()});var F=document.getElementById(`modal-filter-col`),I=document.getElementById(`modal-filter-title`),L=document.getElementById(`modal-filter-desc`),R=document.getElementById(`modal-filter-search`),z=document.getElementById(`modal-filter-options-list`),B=document.getElementById(`modal-filter-selected-count`),V=document.getElementById(`modal-filter-select-all`),H=document.getElementById(`modal-filter-deselect-all`),U=document.getElementById(`modal-filter-close`),W=document.getElementById(`modal-filter-btn-apply`),G=document.getElementById(`modal-filter-btn-clear`),K=[],q=new Set;function J(){B&&(B.innerText=`${q.size} de ${K.length} seleccionados`)}function Y(e=``){if(!z)return;let t=e.toLowerCase().trim(),n=K.filter(e=>!t||e.val.toLowerCase().includes(t));if(n.length===0){z.innerHTML=`<div style="padding: 16px; text-align: center; color: var(--text-muted); font-size: 0.82rem;">No hay opciones que coincidan</div>`;return}z.innerHTML=n.map(e=>{let t=q.has(e.val),n=e.val.replace(/"/g,`&quot;`);return`
        ...
        `}).join(``),z.querySelectorAll(`.filter-option-checkbox`).forEach(e=>{e.addEventListener(`change`,t=>{let n=e.dataset.val;e.checked?q.add(n):q.delete(n),J()})})}document.getElementById(`ctx-filter-col`)?.addEventListener(`click`,()=>{if(N.style.display=`none`,!y)return;let e=p[y]||y;I.innerHTML=`<span class="material-symbols-outlined" style="color: var(--accent-color, #d01212);">filter_alt</span> Filtrar por ${e}`,L.innerText=`Selecciona los valores de "${e}" que deseas visualizar en la tabla:`,R&&(R.value=``);let t=new Map;m.forEach(e=>{let n=C(e,y);t.set(n,(t.get(n)||0)+1)}),K=Array.from(t.entries()).map(([e,t])=>({val:e,count:t})),K.sort((e,t)=>{let n=parseInt(e.val.replace(/\D/g,``),10),r=parseInt(t.val.replace(/\D/g,``),10);return!isNaN(n)&&!isNaN(r)&&y===`numero`?n-r:e.val.localeCompare(t.val,`es`,{numeric:!0})}),q=v[y]&&v[y].size>0?new Set(v[y]):new Set(K.map(e=>e.val)),Y(),J(),F.style.display=`flex`,setTimeout(()=>R?.focus(),50)}),R?.addEventListener(`input`,()=>{Y(R.value)}),V?.addEventListener(`click`,()=>{K.forEach(e=>q.add(e.val)),Y(R?R.value:``),J()}),H?.addEventListener(`click`,()=>{q.clear(),Y(R?R.value:``),J()}),U?.addEventListener(`click`,()=>{F.style.display=`none`}),G?.addEventListener(`click`,()=>{y&&delete v[y],F.style.display=`none`,k()}),W?.addEventListener(`click`,()=>{y&&(q.size===K.length?delete v[y]:v[y]=new Set(q)),F.style.display=`none`,k()});var X=document.getElementById(`mant-search-input`),Z=document.getElementById(`mant-search-clear`);X?.addEventListener(`input`,()=>{Z&&(Z.style.display=X.value?`block`:`none`),k()}),Z?.addEventListener(`click`,()=>{X.value=``,Z.style.display=`none`,k()}),document.getElementById(`btn-refresh-data`)?.addEventListener(`click`,()=>{S()}),document.getElementById(`btn-backup-user-pos`)?.addEventListener(`click`,async()=>{let e=document.getElementById(`btn-backup-user-pos`);try{e.disabled=!0,e.innerHTML=`<span class="material-symbols-outlined">sync</span> Respaldando...`;let t=await u(),n=`✅ Respaldo de Posiciones de Usuario completado:
