...
import{S as e,c as t,f as n,i as r,l as i,m as a,p as o,t as s,u as c,v as l,x as u}from"./preload-helper-44F9sIZn.js";var d=0,f=0,p=console.log;console.log=function(...e){p.apply(console,e);let t=e.join(` `);if(t.includes(`[Firebase] Guardado`)||t.includes(`[Firebase] Agregado`)||t.includes(`[Firebase] Actualizado`)){f++;let e=document.getElementById(`session-writes`);e&&(e.innerText=f)}},window.actualizarMetricasEnVivo=async function(){let t=document.getElementById(`btn-refresh-telemetry`);t&&(t.innerHTML=`<span class="material-symbols-outlined" style="font-size: 1.1rem; animation: spin 1s linear infinite;">sync</span> Refrescando...`);try{await e(u(l,`system_config`,`general`)),d++;let t=document.getElementById(`session-reads`);t&&(t.innerText=d)}catch(e){console.warn(`Lectura offline o sin permiso general:`,e)}setTimeout(()=>{t&&(t.innerHTML=`<span class="material-symbols-outlined" style="font-size: 1.1rem;">refresh</span> Refrescar`)},600)},window.filtrarTabla=function(){let e=(document.getElementById(`filter-table-input`)?.value||``).toLowerCase().trim();document.querySelectorAll(`#tabla-body-consumo tr`).forEach(t=>{let n=t.innerText.toLowerCase();t.style.display=n.includes(e)?``:`none`})},(function(){if(document.getElementById(`nav-wrapper`))return;window.addEventListener(`beforeinstallprompt`,e=>{e.preventDefault(),window.deferredPrompt=e,console.log(`📥 PWA: beforeinstallprompt guardado.`);let t=document.getElementById(`installButton`);t&&(t.style.opacity=`1`,t.style.pointerEvents=`auto`)}),window.addEventListener(`appinstalled`,e=>{console.log(`🎉 PWA: La aplicación fue instalada con éxito.`),window.deferredPrompt=null;let t=document.getElementById(`installButton`);t&&(t.style.opacity=`0.5`,t.style.pointerEvents=`none`)});let e=window.APP_VERSION||localStorage.getItem(`resucito_installed_version`)||`2.1.00`,l=`
    <div id="nav-wrapper">
